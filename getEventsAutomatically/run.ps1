...
foreach ($workload in $workloadOptions) {
    do {
        ...
        if (!$lastPage) {
            Write-Host 'Export-ActivityExplorerData -StartTime $startTime -EndTime $endTime -Filter1 @("Workload", $workload) -PageSize 100 -PageCookie $watermark -OutputFormat Json'
            ...
            Write-Host 'Export-ActivityExplorerData -StartTime $startTime -EndTime $endTime -Filter1 @("Workload", $workload) -PageSize 5000 -OutputFormat Json'
